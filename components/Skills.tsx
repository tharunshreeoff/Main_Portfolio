  ...
  return (
    <section id="skills" className="py-32">
      <div className="section-wrap">
        ...
        <Reveal delay={0.1} className="mt-14 flex flex-wrap gap-2.5">
          {skillGroups.map((g) => (
            <button
              key={g.key}
        ...
        <div className="mt-10 grid gap-x-12 gap-y-6 rounded-[1.25rem] card-border bg-surface/50 p-10 md:grid-cols-2">
          {group.skills.map((s) => (
            <div key={s.name}>
              <div className="mb-2.5 flex items-center justify-between text-[16px]">
        ...
        <Reveal delay={0.2} className="mt-16 overflow-hidden">
          <p className="mb-5 text-center font-mono text-[13px] uppercase tracking-[0.25em] text-muted">
            Full Tech Stack
          </p>
          <div className="relative">
            <div className="flex w-max animate-marquee gap-4">
              {[...techStack, ...techStack].map((t, i) => (
                <span
                  key={t + i}
                  className="whitespace-nowrap rounded-full card-border bg-surface/50 px-5 py-2.5 text-[15px] text-muted"
                >
                  {t}
                </span>
              ))}
